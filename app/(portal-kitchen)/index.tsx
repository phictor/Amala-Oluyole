...
const STATUS_LABEL: Record<string, string> = {
  payment_confirmed: 'New order', accepted: 'Accepted', preparing: 'Preparing', ready: 'Ready',
};
...
function describeCustomMeal(config?: CustomMealConfig | null): string {
  if (!config) return '';
  return [
    config.swallow?.name,
    config.soup?.name,
    ...(config.proteins ?? []).map((item) => `${item.quantity}× ${item.name}`),
    ...(config.extras ?? []).map((item) => `${item.quantity}× ${item.name}`),
  ].filter(Boolean).join(' · ');
}
...
export default function KitchenOrdersScreen() {
  ...
  const kitchenOrders = orders.filter((order) => isKitchenVisibleOrder(order.status));
  ...
  // Alert hook — fires haptic + audio on newly paid, actionable orders.
  useNewOrderAlert(kitchenOrders, muted);

  // Flash banner when new actionable orders arrive (track count changes)
  const prevPendingCount = useRef(0);
  const pendingCount = kitchenOrders.filter((order) => isKitchenNewOrder(order.status)).length;
  useEffect(() => {
  ...
  const pending = kitchenOrders.filter((order) => isKitchenNewOrder(order.status));
  const inProgress = kitchenOrders.filter((order) => ['accepted', 'preparing'].includes(order.status));
  const ready = kitchenOrders.filter((order) => order.status === 'ready');
    ...
    <View key={order.id} style={s.card}>
      <View style={s.cardTop}>
        ...
        <View style={[s.statusBadge, { backgroundColor: (STATUS_COLOR[order.status] ?? '#6B7280') + '25' }]}>
          <Text style={[s.statusText, { color: STATUS_COLOR[order.status] ?? '#6B7280' }]}>{STATUS_LABEL[order.status] ?? order.status}</Text>
        </View>
      </View>
      ...
      <Text style={s.orderMeta}>{order.orderType === 'delivery' ? '🚴 Delivery' : order.orderType === 'dine_in' ? '🍽️ Dine in' : '🏪 Pickup'}</Text>
      {/* Dish list */}
          ...
          {order.items.map((item: any, idx: number) => (
            <View key={idx} style={s.itemRow}>
              ...
              <View style={s.itemInfo}>
                ...
                {item.isCustomMeal && item.customMealConfig ? (
                  <Text style={s.itemConfig}>{describeCustomMeal(item.customMealConfig)}</Text>
                ) : null}
      ...
      {['payment_confirmed', 'accepted', 'preparing'].includes(order.status) && (
        <TouchableOpacity style={s.actionBtn} onPress={() => handleAction(order.id, order.status)} activeOpacity={0.8}>
          <Text style={s.actionBtnText}>
            {isKitchenNewOrder(order.status) ? '✅ Accept' : order.status === 'accepted' ? '👨‍🍳 Start Preparing' : '🍽️ Mark Ready'}
          </Text>
        </TouchableOpacity>
      )}
    </View>
    ...
    <ScreenContainer edges={['top', 'left', 'right']}>
      ...
      <View style={s.header}>
        <View>
          ...
          <Text style={s.sub}>{user?.name ?? 'Kitchen Staff'} · {kitchenOrders.length} active orders</Text>
        </View>
      ...
      >
        {ordersQ.isLoading && <LoadingState message="Loading kitchen orders..." />}
        {ordersQ.isError && <QueryProblem accent="#D97706" title="Order queue unavailable" message="The kitchen queue could not be loaded. Retry before assuming there are no orders." onRetry={() => ordersQ.refetch()} />}
        {pending.length > 0 && (
          <View style={s.group}>
        ...
        {!ordersQ.isLoading && !ordersQ.isError && kitchenOrders.length === 0 && (
          <View style={s.empty}>
...
const s = StyleSheet.create({
  ...
  orderMeta: { fontSize: 14, fontWeight: '600', color: '#374151', marginBottom: 10 },
  ...
  itemConfig: { fontSize: 11, color: '#6B7280', marginTop: 2 },
